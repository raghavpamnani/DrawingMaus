...
<body class="hold-transition login-page" style="background: url('public/images/2.jpg') no-repeat center center fixed;  height:100%; background-size: 100% 100%;">
<div class="login-box">
    ...
    <div class="login-box-body">
        ...
        @if(session()->has('message'))
            <div class="alert alert-warning" id="errorMsg">{{ session()->get('message') }}</div>
        @endif
        <form action="{{ url('/').'/loginWithOtp' }}" method="post">
        		<input type="hidden" name="_method" value="POST">
        		<input type="hidden" name="_token" value="{{ csrf_token() }}">  
            <div class="form-group has-feedback">
                <input type="text" name="email" class="form-control" required placeholder="Email Id" value="{{ session()->get('emailForOtp') }}" >
                <span class="glyphicon glyphicon-envelope form-control-feedback"></span>
            </div>
            <div class="form-group has-feedback">
                <input type="password" class="form-control" placeholder="OTP" name="otp" autofocus="" required>
                <span class="glyphicon glyphicon-lock form-control-feedback"></span>
            </div>
            <div class="row">
                <div class="col-xs-8">  
                    <a href="{{ url('/').'/login' }}" id="loginForm"> Login </a>
                </div>
                <!-- /.col -->
                <div class="col-xs-4">
